...
// This will pass the provided document into stdin of clang-include-fixer to
// ensure it's got the current unsaved version.
function callCIF(args: string[], document: vscode.TextDocument):
    Promise<string> {
      const config = vscode.workspace.getConfiguration(
          'clang-include-fixer', document.uri);
      const binary = config.get('binary') as string;
      return new Promise((resolve, reject) => {
        const full_args =
            args.concat(['-stdin', path.basename(document.fileName)]);
        const params = {cwd: path.dirname(document.fileName)};
        let process =
            cp.execFile(binary, full_args, params, (err, stdout, stderr) => {
              if (err) {
...
async function queryPossibleHeaders(
    query: string, document: vscode.TextDocument):
    Promise<CIFHeaders> {
      const headers_str = await callCIF(['-query-symbol', query], document);
      return JSON.parse(headers_str) as CIFHeaders;
    }

// Pass in a CIFHeaders object with a single header that should be included in
// the document. Will return a string of the entire new document.
function insertHeader(headers: CIFHeaders, document: vscode.TextDocument):
    Promise<string> {
      return callCIF(['-insert-header', JSON.stringify(headers)], document);
    }

// Show quick pick view for user to select a header. Will remove all headers
// from the provided `headers` object to leave just the selected one.
async function askUserToSelectHeader(headers: CIFHeaders):
    Promise<CIFHeaders|undefined> {
      const items: vscode.QuickPickItem[] = [];
      for (const info of headers.HeaderInfos) {
        items.push({label: info.QualifiedName, description: info.Header});
      }
      ...
      headers.HeaderInfos = headers.HeaderInfos.filter((info: any) => {
        return info.Header === selected_item.description;
      });
      return headers;
    }

async function insertIncludeForSymbol(
    symbol: string, editor: vscode.TextEditor) {
  ...
  const position_after =
      position_before.with({line: position_before.line + line_count_change});
  editor.selections = [new vscode.Selection(position_after, position_after)];
}

function getSelectedSymbol(editor: vscode.TextEditor):
    string|undefined {
...
export function activate(context: vscode.ExtensionContext) {
  let disposable =
      vscode.commands.registerCommand('extension.clangIncludeFixer', () => {
        ...
        insertIncludeForSymbol(selected_symbol, editor).catch((error) => {
          vscode.window.showErrorMessage(
              'clang-include-fixer error: ' + error.message);
        });
      });
